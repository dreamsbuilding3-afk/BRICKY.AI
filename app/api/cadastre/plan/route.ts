...
function clean(value: unknown, max = 32) { return typeof value === "string" ? value.trim().slice(0, max) : ""; }
function validCode(value: string) { return /^[0-9A-Z]{2,5}$/i.test(value); }
function validPrefix(value: string) { return /^\d{3}$/.test(value); }
function validSection(value: string) { return /^[A-Z0-9]{1,2}$/i.test(value); }
function validParcel(value: string) { return /^\d{1,4}$/.test(value); }

export async function POST(request: Request) {
  if (!SUPABASE_URL || !SUPABASE_ANON_KEY) return NextResponse.json({ error: "Supabase environment variables are not configured." }, { status: 500 });
  ...
  if (!validCode(commune) || !validPrefix(prefix) || !validSection(section) || !validParcel(parcel)) return NextResponse.json({ error: "Référence cadastrale invalide. Utilise commune, préfixe, section et numéro de parcelle." }, { status: 400 });

  try {
    const propertyResponse = await fetch(`${SUPABASE_URL}/rest/v1/properties?id=eq.${encodeURIComponent(propertyId)}&select=id,user_id`, {
      headers: { apikey: SUPABASE_ANON_KEY, Authorization: authorization }, cache: "no-store",
    });
    const properties = await propertyResponse.json();
    if (!propertyResponse.ok || !Array.isArray(properties) || properties.length !== 1 || !properties[0]?.user_id) return NextResponse.json({ error: "Bien introuvable ou non accessible." }, { status: 404 });

    const planUrl = new URL("https://sandbox.geo.api.gouv.fr/scpc/");
    planUrl.searchParams.set("commune", commune); planUrl.searchParams.set("prefixe", prefix); planUrl.searchParams.set("section", section); planUrl.searchParams.set("parcelle", parcel);
    planUrl.searchParams.set("echelle", "1000"); planUrl.searchParams.set("taille", "A4"); planUrl.searchParams.set("orientation", "paysage");
    ...
    const upsertResponse = await fetch(`${SUPABASE_URL}/rest/v1/property_cadastral?on_conflict=property_id`, {
      method: "POST",
      headers: { apikey: SUPABASE_ANON_KEY, Authorization: authorization, "Content-Type": "application/json", Prefer: "resolution=merge-duplicates,return=representation" },
      body: JSON.stringify(record), cache: "no-store",
    });
    const upsertText = await upsertResponse.text();
    let saved: unknown = null; try { saved = upsertText ? JSON.parse(upsertText) : null; } catch { saved = upsertText; }
