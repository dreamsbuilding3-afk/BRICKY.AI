...
type Extracted = { title?: string | null; description?: string | null; price?: number | null; surface_m2?: number | null; rooms?: number | null; city?: string | null; monthly_rent?: number | null };
...
export default function AnalyzePage() {
  ...
  async function importListing(event: FormEvent) {
    event.preventDefault(); setImporting(true); setError(""); setImportNote("");
    try {
      const response = await fetch("/api/properties/preview", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ url }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data?.error || "Impossible de lire cette annonce.");
      const x = (data.extracted || {}) as Extracted;
      setPayload((p) => ({ ...p, title: x.title || p.title, city: x.city || p.city, price: x.price != null ? String(x.price) : p.price, surface_m2: x.surface_m2 != null ? String(x.surface_m2) : p.surface_m2, rooms: x.rooms != null ? String(x.rooms) : p.rooms, monthly_rent: x.monthly_rent != null ? String(x.monthly_rent) : p.monthly_rent }));
      setImportNote(`${data.extraction?.fields_found ?? 0} données détectées. Vérifie-les avant de lancer l’analyse.`);
  ...
  return <main className="page">
    ...
    <section className="analysis-shell">
      <div className="analysis-intro"><span className="eyebrow">Bricky · V1</span><h1>Est-ce que ce bien mérite votre attention ?</h1><p className="sub">Collez une annonce ou entrez les données que vous connaissez. Bricky calcule, vérifie et signale ce qui manque — sans inventer.</p></div>
      <form className="import-form" onSubmit={importListing}><div className="import-copy"><b>⚡ Analyser une annonce</b><span>Collez l’URL d’une annonce immobilière</span></div><div className="import-row"><input type="url" required value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://…" /><button className="secondary-button" disabled={importing}>{importing ? "Lecture…" : "Importer"}</button></div>{importNote && <div className="import-note">✓ {importNote}</div>}</form>
      <div className="manual-label"><span>ou compléter manuellement</span></div>
...
function AnalysisDashboard({ result }: { result: AnalysisResult }) {
  const analysis = (result.analysis as Record<string, any>) || result; const financial = (analysis.financial_snapshot || {}) as Record<string, any>; const decision = (analysis.decision_snapshot || {}) as Record<string, any>; const risk = (analysis.risk || {}) as Record<string, any>; const scenarios = (analysis.financial_scenarios || {}) as Record<string, any>; const verdict = decision.verdict || analysis.verdict; const score = decision.score ?? analysis.overall_score; const confidence = decision.confidence_score ?? analysis.confidence_score; const actions = Array.isArray(decision.actions) ? decision.actions : []; const risks = Array.isArray(risk.risks) ? risk.risks : []; const missing = Array.isArray(analysis.missing_information) ? analysis.missing_information : []; const rows = ["base", "conservative", "optimistic"].filter((k) => scenarios[k]); const label = verdict === "interesting" ? "Intéressant" : verdict === "unattractive" ? "Peu intéressant" : "À vérifier & négocier";
  return <section className="result-panel decision-dashboard"><div className="result-head"><div><span className="eyebrow">Analyse terminée</span><h2>Voici ce que Bricky en pense.</h2></div><span className="status-dot">● Décision</span></div><div className="decision-hero"><div><span className="decision-label">Verdict</span><strong>{label}</strong></div><div className="score-block"><span>Score</span><b>{score ?? "—"}<small>/100</small></b></div><div className="score-block"><span>Confiance</span><b>{confidence ?? "—"}<small>%</small></b></div></div><div className="metric-grid"><Metric label="Loyer mensuel" value={financial.monthly_rent} suffix=" €" /><Metric label="Revenu annuel" value={financial.annual_net_income ?? financial.annual_gross_income} suffix=" €" /><Metric label="Rendement brut" value={financial.gross_yield} suffix=" %" /><Metric label="Rendement net" value={financial.net_yield} suffix=" %" /></div>{rows.length > 0 && <div className="dashboard-section"><h3>Scénarios</h3><div className="scenario-grid">{rows.map((key) => <div className="scenario" key={key}><span>{key === "base" ? "Base" : key === "conservative" ? "Conservateur" : "Optimiste"}</span><b>{scenarios[key].net_yield ?? "—"} %</b><small>rendement net</small></div>)}</div></div>}{actions.length > 0 && <div className="dashboard-section"><h3>Ce qu’il faut faire</h3><ul>{actions.map((a: string, i: number) => <li key={i}>{a}</li>)}</ul></div>}{risks.length > 0 && <div className="dashboard-section"><h3>Points de vigilance</h3><div className="risk-list">{risks.map((r: any, i: number) => <div className="risk-item" key={i}><b>{r.title || "Risque"}</b><span>{r.severity || ""}</span><p>{r.explanation || ""}</p></div>)}</div></div>}{missing.length > 0 && <div className="dashboard-section"><h3>Données manquantes</h3><div className="missing-list">{missing.map((m: any, i: number) => <div key={i}><b>{m.label || m.field_key}</b><p>{m.suggested_question || m.impact || "À vérifier avant décision."}</p></div>)}</div></div>}</section>;
}
